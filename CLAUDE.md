...
Progress so far covers build steps 1-3. Notable departures from what's written above:

- **Step 3 (transfer planner) is scoped to one-for-one, same-position swaps.** For any future gameweek, you can transfer a squad player out for any other real player of the same position from the full FPL player pool, tracked against bank/budget, with free-transfer rollover and the -4-per-extra-transfer hit. What's *not* supported: multi-transfer baskets that temporarily break squad composition (e.g. sell 2 DEF, buy 2 MID), and wildcard/free-hit (unlimited free transfers) — the latter is chip strategy, build step 4. A few further simplifications, each disclosed rather than hidden:
  ...
  - **Free-transfer availability is replayed from the manager's real FPL history** (`GET entry/{id}/history/`, imported per team): actual transfers made each past gameweek plus any Wildcard/Free Hit gameweeks (which don't consume or roll the saved free transfer) are walked forward through FPL's real rollover rule from gameweek 2 to today, then continued through this planner's own saved plans for future gameweeks. Not yet handled: a manager who plays a chip *through this planner* for a future gameweek — chip strategy itself is still build step 4, unbuilt.
