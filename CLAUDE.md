...
- **`freeTransferCap`/`freeTransferRolloverLimit` (1 and 5) are confirmed correct** against the live 2026/27 season FPL rules (1 free transfer/gameweek, rollover capped at 5, no mid-season bump this season) — no longer an open assumption, unlike the risk originally flagged in the "Classic FPL rules to encode" section above. Still hardcoded rather than read live like the chip set is, so re-verify if a season boundary passes.
...
- **The player search panel's info icon opens a profile card** (season totals, form, ICT Index, expected goals/assists, ownership, and a portrait) pulled from the same FPL bootstrap API response already used for prices/status, refreshed on the same "next import" cadence — not live-polled, so it can lag a few hours behind FPL's own site right after a game. The portrait and stat definitions (ICT Index, xG, xA) are FPL's own, not independently recomputed here. **Some portraits show a player in an old club's kit after a transfer** (confirmed on Cunha and Isak, both mid-2025 moves) — this is stale artwork on Premier League's own photo CDN (`resources.premierleague.com`, the same host the official FPL site itself hotlinks), not a caching or URL-building issue here: the club/team data itself is correct and current, only the photo asset lags. Nothing to fix on this app's side short of dropping portraits entirely or building a separate photo pipeline, neither of which seems worth it for a cosmetic, self-correcting-over-time upstream issue.
...
- **A pitch/bench card shows a player's actual points once their fixture has finished, instead of who they're playing** — but only for the real live current gameweek, and only once *every* fixture that club has in it (there can be more than one, on a double gameweek) is finished. Sourced from the FPL API's `/event/{id}/live/` endpoint (`FplClient.get_event_live`), fetched alongside bootstrap on every import and stored as `Player.currentGameweekPoints` — a refreshed snapshot of "whatever gameweek was current at last import," the same treatment as `currentPrice`, not a historical per-gameweek points table. `Fixture.finished` gets the same on-import refresh, sourced from a field the importer previously fetched but didn't parse.
